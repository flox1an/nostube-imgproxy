# Lionfish NIP-98 media URL minting

## Goal

Lionfish issues short-lived HMAC capability URLs directly to the Nostube browser application. The browser authenticates the **mint** request with NIP-98; it never receives an HMAC signing key. The returned URL is then loaded normally through `img`, `video poster`, preload, or metadata markup.

```text
Browser -- NIP-98 POST /v1/mint --> Lionfish -- HMAC URL --> Browser
Browser -------- GET /v1/{key-id}/{signature}/thumb/... ----------> Lionfish
```

The HMAC secret exists only in Lionfish's deployment secrets:
...
`MINT_PUBLIC_BASE_URL` is the canonical public Lionfish origin. Lionfish uses it to validate NIP-98's exact `u` tag and to construct returned URLs; it is never inferred from `Host` or forwarding headers.
...
| `MINT_PUBLIC_BASE_URL` | unset | Required HTTPS Lionfish origin in production; exact NIP-98 request target |
...
| `SIGNED_URL_TTL_SECS` | `21600` | Minted URL lifetime; Lionfish rounds expiry to a stable TTL bucket |
...
- have an event ID that Lionfish has not accepted during `NIP98_REPLAY_TTL_SECS`.
...
- `extension` is optional for images. Supported explicit image extensions are `jpg`, `jpeg`, `png`, and `webp`; supported direct-container video extensions are Lionfish's FFmpeg allowlist, such as `mp4`, `webm`, and `mkv`.
...
The request is atomic. Invalid JSON or any invalid item returns `400`; Lionfish mints no URLs. Lionfish does not fetch blobs while minting.
...
The subsequent image or video-poster load is a standard unauthenticated `GET` to Lionfish. The v1 URL is the time-bounded capability. Lionfish verifies its HMAC and `exp` before fetching, decoding, or invoking FFmpeg.
...
The browser must not attempt to recreate HMAC signatures and must not retain Lionfish signing material.

## Security boundary

All Nostr pubkeys are initially eligible to mint URLs. A pubkey is therefore an attribution and fairness key, not a scarce authorization credential: new keys are cheap to generate. Lionfish enforces both IP and pubkey budgets, charges by item rather than request, uses replay protection, and allows only fixed presets plus hash-addressed input.
...
1. Deploy Lionfish with signed v1 routes, a signing key, and `ALLOW_UNSIGNED_URLS=true`.
